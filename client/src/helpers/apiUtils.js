...
export const sendPrompt = async (prompt) => {
    try {
        ...
        const response = await axios.post(`http://${URL}:5000/api/prompt`, { prompt: prompt + ' ' + preferences });
...
export const getNext = async () => {
    ...
    try {
        const response = await axios.get(`http://${URL}:5000/api/next`);
        return response.data;
    } catch (error) {
        console.error(error);
    }
